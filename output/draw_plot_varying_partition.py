...
marker=['x','o',"v",'*','s',"p",'D','^']
x = [1,2,3,4,5]
my_xticks = ['$1$','$4$','$8$','$16$','$32$']
df = pd.read_csv('vary_partition',header=-1, sep='\s+',)
...
pylab.xlabel(r"$\#$"+ "partition, "+r"$\tau$",fontsize=fontsize)
...
if(show):
	pylab.show()
pylab.savefig("test_accuracy_vary_partition.pdf")
pylab.clf()
...
if(show):
	pylab.show()
pylab.savefig("train_accuracy_vary_partition.pdf")
pylab.clf()
...
if(show):
	pylab.show()
pylab.savefig("time_vary_partition.pdf")
pylab.clf()
...
if(show):
	pylab.show()
pylab.savefig("rule_size_vary_partition.pdf")
pylab.clf()
